...
Figura adicionaModeloVBO(const char* nome, GLuint *buffers, int numIndices, unsigned short *indices, Figura lista){
...
Figura parsingModeloVBO(const char* filename, Figura lista){
	...
	return adicionaModeloVBO(filename, buffers, numIndices, indices, lista);
}
...
void desenhaModeloVBO(ModeloVBO modelo) {
	...
	// glDrawElements(GL_TRIANGLES, modelo->numIndices, GL_UNSIGNED_SHORT, modelo->indices);
