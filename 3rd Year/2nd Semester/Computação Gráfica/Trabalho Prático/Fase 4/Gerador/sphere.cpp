...
void sphereVBO(float raio, int fatias, int camadas, string nomeFicheiro) {
	...
	for (contadorFatias=0; contadorFatias<fatias; contadorFatias++){
		...
		indices[indice++] = 0;
		indices[indice++] = contadorFatias + 1;
		indices[indice++] = contadorFatias + 2;
	...
	for (contadorCamadas=1; contadorCamadas<camadas-1; contadorCamadas++){
		...
		for (contadorFatias = 0; contadorFatias<fatias; contadorFatias++){
			...
			indices[indice++] = avanco - fatias + contadorFatias;
			indices[indice++] = avanco + contadorFatias;
			indices[indice++] = avanco - fatias + contadorFatias + 1;

			indices[indice++] = avanco + contadorFatias;
			indices[indice++] = avanco + contadorFatias + 1;
			indices[indice++] = avanco - fatias + contadorFatias + 1;

			angCircular2 += anguloCircular;
		}

		indices[indice - 4] = avanco - fatias;
		indices[indice - 2] = avanco;
		indices[indice - 1] = avanco - fatias;
	...
	for (contadorFatias = 0; contadorFatias < fatias; contadorFatias++) {
		...
		indices[indice++] = avanco + contadorFatias;
		indices[indice++] = avanco + fatias;
		indices[indice++] = avanco + contadorFatias + 1;
	}
	indices[indice - 1] = avanco;
	...
	f << numIndices << "\n";
	for (indice = 0; indice < numIndices; indice += 3)
		f << fixed << setprecision(6) << indices[indice] << " " << indices[indice + 1] << " " << indices[indice + 2] << "\n";
}