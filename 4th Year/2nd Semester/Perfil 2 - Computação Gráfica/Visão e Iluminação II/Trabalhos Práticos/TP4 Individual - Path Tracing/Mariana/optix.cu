...
struct RadiancePRD{
    float3   emitted; //luz emitida pelas fontes primárias
    float3   radiance; //luz visível 
    float3   attenuation; //perda gradual de intensidade de qualquer tipo de fluxo através de um meio
    float3   origin; //origem da luz
    float3   direction; //direção da luz
    bool done;
    uint32_t seed;
...
extern "C" __global__ void __closesthit__radiance_metal() {
    ...
    prd.seed = seed;
    prd.radiance +=  (glossy *make_float3(0.8f, 0.8f, 0.8f) / glossy_rays);
}
  ...
  //  float3 normal = normalize(make_float3(n));
    ...
    if (dotP > 0) {
       // float3 reflectDir = reflect(normRayDir, normal);
        ...
        prd.radiance += make_float3(5.0f, 5.0f, 5.0f)*(weight)*optixLaunchParams.global->lightScale * (refractPRD.radiance * (refractPRD.radiance * (1-r0) + r0*refractPRD.radiance));
